    ...
    while (true) {
        try {
            ...
            if (real !== rootReal && !real.startsWith(rootReal + path.sep)) {
                throw new Error(
                    `credHome resolves outside workspace through symlink: ${credHome}`,
                );
            }
            return;
        } catch (err) {
...
class AcpClient {
    constructor(agentCfg, workDir, opts = {}) {
        ...
        this._debug(
            `spawned pid=${this._pid} command=${command} args=${JSON.stringify(args)}`,
        );
        ...
        this.proc.on("exit", (code, signal) => {
            ...
            if (code !== 0 && code !== null) {
                const snippet = this._stderrText
                    ? `: ${redact(this._stderrText).trim()}`
                    : "";
                process.stderr.write(
                    `[${this.name}] exited with code ${code}${snippet}\n`,
                );
            }
            fail(err);
        });
        ...
        this.proc.stdin.on("error", (err) =>
            fail(new Error(`[${this.name}] stdin: ${err.message}`)),
        );
        ...
        this.proc.stdout.on("data", (chunk) => {
            ...
            while ((nl = buf.indexOf("\n", bufStart)) !== -1) {
                ...
                if (utf8ByteLength(line) > this._maxLine) {
                    fail(
                        new Error(
                            `[${this.name}] stdout line exceeded ${this._maxLine} bytes`,
                        ),
                    );
                    return;
                }
            ...
            if (utf8ByteLength(buf.slice(bufStart)) > this._maxLine) {
                fail(
                    new Error(
                        `[${this.name}] unterminated stdout line exceeded ${this._maxLine} bytes`,
                    ),
                );
            }
        });
        ...
        this.proc.stderr.on("data", (chunk) => {
            ...
            if (this._stderrBytes > this._maxOutput) {
                fail(
                    new Error(
                        `[${this.name}] stderr exceeded ${this._maxOutput} bytes`,
                    ),
                );
                return;
            }
            ...
            if (this._onStderrLine || this._debugLog) {
                ...
                while ((match = this._stderrLineBuf.match(/[\r\n]/)) !== null) {
                    ...
                    if (
                        char === "\r" &&
                        this._stderrLineBuf[idx + 1] === "\n"
                    ) {
                        this._stderrLineBuf = this._stderrLineBuf.slice(
                            idx + 2,
                        );
                    } else {
                        this._stderrLineBuf = this._stderrLineBuf.slice(
        ...
        if (msg.method) {
            const params =
                msg.params !== undefined
                    ? ` params=${truncate(redact(JSON.stringify(msg.params)))}`
                    : "";
    ...
    prompt(sessionId, content, timeoutMs = 0) {
        ...
        const appendOutput = (chunk, source) => {
            ...
            if (accumulatedBytes > this._maxOutput) {
                this._fail(
                    new Error(
                        `[${this.name}] ${source} output exceeded ${this._maxOutput} bytes`,
                    ),
                );
                return false;
            }
        ...
        const collector = (msg) => {
            ...
            if (u.usage) {
                totalInputTokens +=
                    u.usage.input_tokens || u.usage.prompt_tokens || 0;
                totalOutputTokens +=
                    u.usage.output_tokens || u.usage.completion_tokens || 0;
            }
        };
        ...
        return this._rpc(
            ...
            (result) => {
                ...
                for (const block of result?.content || []) {
                    ...
                    if (text === false) {
                        throw (
                            this._dead ||
                            new Error(
                                `[${this.name}] direct response output exceeded ${this._maxOutput} bytes`,
                            )
                        );
                    }
                    direct += text;
                ...
                function mergePromptText(streamed, direct) {
                    const a = String(streamed || "");
                    const d = String(direct || "");
                    if (!a) return d;
                    if (!d) return a;
                    ...
                    const dt = d.trim();
                    if (at === dt) return a;
                    if (d.includes(a)) return d;
                    if (a.includes(d)) return a;

                    return `${a}${a.endsWith("\n") || d.startsWith("\n") ? "" : "\n"}${d}`;
                }

                const text = mergePromptText(accumulated, direct);
                const usage =
