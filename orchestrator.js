...
if (debugLog)
    debugLog(
        "debug logging enabled — full ACP frames + sub-agent stderr will be mirrored to stderr",
    );
...
async function sleepInterruptible(ms, isCancelled) {
    ...
    while (performance.now() < end) {
        ...
        await new Promise((r) =>
            setTimeout(r, Math.max(0, Math.min(100, end - performance.now()))),
        );
    }
}
...
const DECISION_RE = /^[\s>*#`]*(APPROVED|QUESTIONS)\b[ \t]*:?[ \t]*/im;
const INJECT_RE = /^([ \t>*#`]*)(APPROVED|QUESTIONS)\b([ \t]*:?)/gim;

function parseReviewerDecision(text) {
    const raw = String(text || "");
    const match = DECISION_RE.exec(raw);

    if (!match) {
        return { kind: null, body: raw.trim() };
    }

    return {
        kind: match[1].toLowerCase(),
        body: raw.slice(match.index + match[0].length).trim(),
        prefix: raw.slice(0, match.index).trim(),
    };
}
...
function taskToText(task) {
    const blocks = Array.isArray(task)
        ? task
        : [{ type: "text", text: String(task || "") }];
    return blocks
        .map((b) => b.text || "")
        .join("\n")
        .trim();
...
function appendTextBlock(blocks, text) {
    return [
        ...(Array.isArray(blocks)
            ? blocks
            : [{ type: "text", text: String(blocks || "") }]),
        { type: "text", text },
    ];
}

function truncateForReviewer(text) {
    if (!REVIEWER_AGENT_CHARS || text.length <= REVIEWER_AGENT_CHARS)
        return text;
    return (
        text.slice(0, REVIEWER_AGENT_CHARS) +
...
const GROUP_DIRECTIVE_RE =
    /^[ \t]*@orchestrator[ \t]+(?:group|mode)[ \t]*:[ \t]*([A-Za-z0-9_.-]+)[ \t]*$/im;
...
function requestedGroupName(task, groupNames = Object.keys(AGENT_GROUPS)) {
    ...
    const m =
        first.line.match(GROUP_DIRECTIVE_RE) ||
        (slashRe ? first.line.match(slashRe) : null);
    return m ? m[1] : null;
}
...
function stripGroupDirective(task, groupNames = Object.keys(AGENT_GROUPS)) {
    ...
    return blocks.map((block, index) => {
        if (index !== first.blockIndex || typeof block.text !== "string")
            return block;
...
function realInside(parentReal, childReal) {
    return (
        childReal === parentReal || childReal.startsWith(parentReal + path.sep)
    );
}
...
function resolveArtifactRootForRead(workDir, artifactDir) {
    if (path.isAbsolute(artifactDir)) {
        throw new Error(
            `artifactDir must be workspace-relative: ${artifactDir}`,
        );
    }
    ...
    if (!realInside(workspaceReal, rootReal)) {
        throw new Error(
            `artifactDir resolves outside workspace through symlink: ${artifactDir}`,
        );
    }
...
function readApprovedPlanFileWithinRoot(resolvedRoot, file) {
    ...
    if (!realInside(resolvedRoot.rootReal, targetReal)) {
        throw new Error(
            `approved plan resolves outside artifact root: ${file}`,
        );
    }
...
function findLatestApprovedPlan(workDir, artifactDir, groupName, onWarning) {
    ...
    try {
        ...
        if (!isExpectedScanRace(err)) {
            onWarning?.(
                `approved-plan scan failed for ${artifactDir}: ${err.message}`,
            );
        }
        return null;
    }
    ...
    for (const runDir of entries) {
        ...
        try {
            ...
            const text = readApprovedPlanFileWithinRoot(
                resolvedRoot,
                candidate,
            );
            ...
            onWarning?.(
                `approved-plan candidate skipped (${path.relative(root, candidate)}): ${err.message}`,
            );
        }
    }
    return latest;
...
async function probeAgent(
    ...
    try {
        ...
        if (isCancelled?.()) throw new Error("CANCELLED");
        notifyZed?.(
            `> **${agentCfg.name}** — probe failed: ${redact(err.message)} (will skip)\n`,
        );
        return false;
    } finally {
        unregister?.();
...
async function runAgent(
    ...
    await rateLimiter.acquire(
        agentCfg,
        (waitMs) => onRateLimit?.(waitMs, rateLimitKey),
        isCancelled,
    );
    ...
    const remaining = (phase) => {
        ...
        if (left <= 0) {
            const err = new Error(
                `TIMEOUT:${totalTimeoutMs}ms — ${agentCfg.name} exceeded agentTimeoutMs before ${phase}`,
            );
            throw err;
        }
    ...
    const onStderrLine = onProviderError
        ? (line) => {
    ...
    try {
        const initResult = await client.init(
            maskClientCapabilities(zedClientCapabilities || {}, policy),
            remaining("initialize"),
        );
        const agentCapabilities =
            initResult?.agentCapabilities || initResult?.capabilities || {};
        ...
        if (filtered.dropped.length) {
            onWarning?.(
                `${agentCfg.name} does not advertise support for prompt block(s): ${filtered.dropped.join(", ")}. Those blocks were not sent to this child.`,
            );
        }
        const mcpFiltered =
            policy?.mcp === false
                ? {
                      kept: [],
                      dropped: (mcpServers || []).map(
                          (server) => server.name || server.type || "unknown",
                      ),
                  }
        ...
        if (mcpFiltered.dropped.length) {
            onWarning?.(
                `${agentCfg.name} does not advertise support for MCP server(s): ${mcpFiltered.dropped.join(", ")}. Those servers were not sent to this child.`,
            );
        }
        const sid = await client.newSession(
            workDir,
            mcpFiltered.kept,
            remaining("session/new"),
        );
        return await client.prompt(
            sid,
            filtered.content,
            remaining("session/prompt"),
        );
    } finally {
...
async function orchestrate(
    ...
    const subAgents =
        group.strategy === "single_writer"
            ? [
                  group.subAgents.find((a) => a.name === group.writer) ||
                      group.subAgents[0],
              ]
            : group.subAgents;
    ...
    const proxyChildRequest = (agentCfg, policy, retryState) => (msg) => {
        ...
        return ctx.rpcToZed(
            msg.method,
            rewriteSessionId(msg.params, sessionId),
        );
    };
    ...
    if (group.strategy === "single_writer" && group.attachApprovedPlanFrom) {
        ...
        if (!planFile) {
            planFile = findLatestApprovedPlan(
                workDir,
                artifactDir,
                sourceGroup,
                (warning) =>
                    notifyText(sessionId, `> Warning: ${redact(warning)}\n`),
            );
            planMeta = null;
        }
        if (planFile) {
            try {
                const plan = readApprovedPlan(
                    ...
                    artifactDir,
                    planFile,
                    planMeta?.sha256 || null,
                );
                const alreadyMentioned =
                    taskToText(task).includes("approved-plan");
                if (!alreadyMentioned) {
                    if (!fromCurrentSession) {
                        notifyText(
                            sessionId,
                            `> Warning: using latest approved plan found on disk, not a plan from this live session.\n`,
                        );
                    }
                    task = appendTextBlock(
                        task,
                    ...
                    notifyText(
                        sessionId,
                        `> Attached approved plan from ${path.relative(workDir, planFile)}\n`,
                    );
                }
            } catch (err) {
                notifyText(
                    sessionId,
                    `> Warning: approved plan could not be attached: ${redact(err.message)}\n`,
                );
            }
        }
    }

    await writeInputPrompt(
        workDir,
        artifactDir,
        runId,
        redact(taskToText(task)),
        {
            sessionId,
            promptId,
            group: group.name,
    ...
    for (let turn = 1; turn <= groupMaxTurns; turn++) {
        if (isCancelled())
            return { text: "**Cancelled by user.**", approved: false };
        ...
        const subPrompt = questions
            ? appendTextBlock(
                  instructedTask,
                  `## Open questions - address ALL of them\n${questions}`,
              )
            : instructedTask;
        ...
        if (PROBE_TIMEOUT_MS > 0 && turn === 1 && subAgents.length > 1) {
            const registerProbe = (client) => {
                ctx.clients.add(client);
                return () => ctx.clients.delete(client);
            };
            const probeSettled = await Promise.allSettled(
                subAgents.map(async (cfg) => {
            ...
            if (!activeAgents.length) {
                return {
                    approved: false,
                    text: "**All agents failed the startup health probe.**",
                };
            }
        ...
        const settled = await Promise.allSettled(
            activeAgents.map((cfg) => {
                const policy = effectivePolicyForAgent(
                    group,
                    cfg,
                    group.strategy === "single_writer" ? "writer" : "subagent",
                );
                ...
                const retryState = {
                    hadSideEffect: group.strategy === "single_writer",
                };
                    ...
                    () =>
                        runAgentWithRetry(
                            ...
                            (elapsedSec) => {
                                notifyText(
                                    sessionId,
                                    `> **${cfg.name}** — still working (${elapsedSec}s elapsed)\n`,
                                );
                            },
                            ...
                            (code, line) => {
                                notifyText(
                                    sessionId,
                                    `> **${cfg.name}** — provider HTTP ${code}: ${line}\n`,
                                );
                            },
                        ),
                    isCancelled,
        ...
        const fatalErrors = subResults.filter(
            (r) => r.fatal && extractHttpStatus(r.error) !== null,
        );
        ...
        const failedNote = subResults.filter((r) => r.error).length
            ? `\n> Note: the following agents did not respond and are excluded: ${subResults
                  .filter((r) => r.error)
                  .map((r) => r.name)
                  .join(", ")}\n`
            : "";
        ...
        const agentResultsSection = [
            ...
            ...subResults
                .filter((r) => !r.error)
                .map(({ name, result }) => {
                    const safeName = escapeForReviewerReport(name).replace(
                        /"/g,
                        "&quot;",
                    );
...
1. <remaining gaps only>`;
        ...
        await writeReviewerPrompt(
            workDir,
            artifactDir,
            runId,
            group.name,
            turn,
            redact(reviewPromptText),
            { sessionId, promptId },
        );
        const reviewerPolicy = effectivePolicyForAgent(
            group,
            reviewer,
            "reviewer",
        );
        ...
        try {
            reviewerOutcome = await runAgentWithRetry(
                ...
                (elapsedSec) => {
                    notifyText(
                        sessionId,
                        `> **${reviewer.name}** — still working (${elapsedSec}s elapsed)\n`,
                    );
                },
                ...
                (code, line) => {
                    notifyText(
                        sessionId,
                        `> **${reviewer.name}** — provider HTTP ${code}: ${line}\n`,
                    );
                },
            );
        } catch (err) {
            ...
            notifyText(
                sessionId,
                `> **Reviewer failed permanently**: ${reviewerErrMsg}\n`,
            );
        ...
        if (
            reviewerOutcome?.stopReason &&
            reviewerOutcome.stopReason !== "end_turn"
        ) {
            notifyText(
                sessionId,
                `> **Reviewer stopped with ${reviewerOutcome.stopReason}**\n`,
            );
            return {
                approved: false,
                text: "**Reviewer did not complete normally.**",
            };
        }
        ...
        const decision = parseReviewerDecision(reviewResult);

        if (decision.kind === "approved") {
            const approvedText = decision.body;
            const sha256 = approvedPlanHash(approvedText);
            const approvedPath = await writeApprovedPlan(
                workDir,
                artifactDir,
                runId,
                group.name,
                approvedText,
                { sessionId, promptId, sha256 },
            );
            const planMeta = {
                path: approvedPath,
                sha256,
        ...
        questions =
            decision.kind === "questions" ? decision.body : reviewResult.trim();
