...
function decoratePromptResult(result, notifs) {
    return {
        ...
        content: result?.content || [
            { type: "text", text: streamedText(notifs) },
        ],
    };
}
...
async function cleanupOrchestratorProcess({
    proc,
    rl,
    pending,
    state,
...
function buildConfig(tmpDir, { subResponses, reviewerResponse, maxTurns = 2 }) {
    ...
    fs.writeFileSync(
        cfgPath,
        JSON.stringify({ maxTurns, subAgents, reviewer }),
    );
    return cfgPath;
}
...
function runOrchestrator(cfgPath, workDir, opts = {}) {
    return new Promise((resolve, reject) => {
        ...
        const finish = async (fn, value) => {
            if (settled) return;
            settled = true;
            await cleanupOrchestratorProcess({
                proc,
                rl,
                pending,
                state,
                ...
                Promise.resolve()
                    ...
                    .catch((err) => {
                        ...
                        safeWriteFrame(proc, {
                            ...
                            id: msg.id,
                            error: {
                                code: Number.isInteger(err?.code)
                                    ? err.code
                                    : -32000,
        ...
        timeout = setTimeout(() => {
            void finish(
                reject,
                new Error(
                    `orchestrator helper timeout (${opts.timeoutMs || ORCHESTRATOR_HELPER_TIMEOUT_MS}ms)`,
                ),
            );
            ...
            try {
                ...
                const { sessionId } = await rpc("session/new", {
                    ...
                    ...(opts.sessionMcpServers
                        ? { mcpServers: opts.sessionMcpServers }
                        : {}),
                });
        ...
        (async () => {
            try {
                ...
                for (const [index, prompt] of prompts.entries()) {
                    const raw = await rpc("session/prompt", {
                        sessionId,
                        prompt,
                    });
    ...
    return new Promise((resolve, reject) => {
        const timer = setTimeout(
            () =>
                reject(
                    new Error(
                        `TEST TIMEOUT (${PER_TEST_TIMEOUT_MS}ms): ${fn.name} hung`,
                    ),
                ),
            PER_TEST_TIMEOUT_MS,
        );
...
async function test_approved_on_first_round() {
    ...
    const cfgPath = buildConfig(tmpDir, {
        subResponses: [
            "Here is the function: function hello() {}",
            "def hello(): pass",
        ],
        reviewerResponse:
            "APPROVED: Use hello() as the canonical implementation.",
        maxTurns: 3,
    });
    ...
    assert(
        "initialize response includes authMethods",
        Array.isArray(initResponse?.result?.authMethods),
    );
    assert(
        "initialize advertises image prompt capability",
        initResponse?.result?.agentCapabilities?.promptCapabilities?.image ===
            true,
        JSON.stringify(initResponse?.result?.agentCapabilities),
    );
    assert(
        "initialize advertises HTTP MCP forwarding capability",
        initResponse?.result?.agentCapabilities?.mcpCapabilities?.http === true,
        JSON.stringify(initResponse?.result?.agentCapabilities),
    );
    assert(
        "prompt response omits legacy content field",
    ...
    const roundNotifs = notifs.filter((n) =>
        n.params?.update?.content?.text?.includes("Round"),
    );
    assert(
        "only 1 round executed",
        roundNotifs.length === 1,
        `got ${roundNotifs.length}`,
    );
    const chunkNotif = notifs.find(
        (n) => n.method === "session/update" && n.params?.update?.content?.text,
    );
    assert(
        "session/update uses sessionUpdate discriminator",
...
`,
    ...
    assert(
        "streamed chunk reached reviewer prompt",
        text.includes("streamed-ok"),
        `got: ${text}`,
    );
...
`,
    ...
    assert(
        "permission request was not sent to Zed",
...
async function test_invalid_config_fails_fast() {
    ...
    assert(
        "unknown top-level key exits non-zero",
        unknownTop.code !== 0,
        `code=${unknownTop.code}`,
    );
    assert(
        "unknown top-level key message is specific",
    ...
    assert(
        "misspelled group permissions exits non-zero",
        misspelledGroupPermission.code !== 0,
        `code=${misspelledGroupPermission.code}`,
    );
    assert(
        "misspelled group permissions message is specific",
        misspelledGroupPermission.stderr.includes(
            'agentGroups.plan contains unknown key "permissons"',
        ),
    ...
    assert(
        "unknown group key exits non-zero",
        unknownGroup.code !== 0,
        `code=${unknownGroup.code}`,
    );
    assert(
        "unknown group key message is specific",
        unknownGroup.stderr.includes(
            'agentGroups.plan contains unknown key "unknownGroupKey"',
        ),
        unknownGroup.stderr,
    );
    ...
    assert(
        "unknown agent key exits non-zero",
        unknownAgent.code !== 0,
        `code=${unknownAgent.code}`,
    );
    assert(
        "unknown agent key message is specific",
        unknownAgent.stderr.includes(
            'subAgents[0] contains unknown key "unknownAgentKey"',
        ),
        unknownAgent.stderr,
    );
    ...
    assert(
        "unknown MCP server key exits non-zero",
        unknownMcp.code !== 0,
        `code=${unknownMcp.code}`,
    );
    assert(
        "unknown MCP server key message is specific",
        unknownMcp.stderr.includes(
            'mcpServers[0] contains unknown key "unknownMcpKey"',
        ),
        unknownMcp.stderr,
    );
    ...
    assert(
        "unknown rate-limit key exits non-zero",
        unknownRateLimit.code !== 0,
        `code=${unknownRateLimit.code}`,
    );
    assert(
        "unknown rate-limit key message is specific",
        unknownRateLimit.stderr.includes(
            'rateLimits.node contains unknown key "unknownLimitKey"',
        ),
    ...
    assert(
        "bad rate limit exits non-zero",
        badRateLimit.code !== 0,
        `code=${badRateLimit.code}`,
    );
    assert(
        "bad rate limit message is specific",
    ...
    assert(
        "bad MCP server exits non-zero",
        badMcp.code !== 0,
        `code=${badMcp.code}`,
    );
    assert(
        "bad MCP server message is specific",
        ...
        mcpServers: [
            {
                ...
                name: "lazy-mcp",
                url: "https://example.invalid/mcp",
                headers: [{ name: "Authorization", value: 123 }],
            },
        ],
    });
    assert(
        "bad MCP header exits non-zero",
        badMcpHeader.code !== 0,
        `code=${badMcpHeader.code}`,
    );
    assert(
        "bad MCP header message is specific",
    ...
    assert(
        "bad MCP env exits non-zero",
        badMcpEnv.code !== 0,
        `code=${badMcpEnv.code}`,
    );
    assert(
        "bad MCP env message is specific",
    ...
    const badAgentEnv = await runInvalidConfig({
        ...base,
        subAgents: [
            { name: "A", command: "node", args: [], env: { TOKEN: 123 } },
        ],
    });
    assert(
        "bad agent env exits non-zero",
        badAgentEnv.code !== 0,
        `code=${badAgentEnv.code}`,
    );
    assert(
        "bad agent env message is specific",
    ...
    assert(
        "bad workflow exits non-zero",
        badWorkflow.code !== 0,
        `code=${badWorkflow.code}`,
    );
    assert(
        "bad workflow message is specific",
        badWorkflow.stderr.includes(
            "workflow[1] references unknown agent group",
        ),
        badWorkflow.stderr,
    );
    ...
    assert(
        "write-capable parallel_reports exits non-zero",
        badParallelPolicy.code !== 0,
        `code=${badParallelPolicy.code}`,
    );
    assert(
        "write-capable parallel_reports error is specific",
        badParallelPolicy.stderr.includes(
            "parallel_reports groups must be read-only",
        ),
    ...
    assert(
        "parallel_reports writeFiles object exits non-zero",
        badParallelWriteObject.code !== 0,
        `code=${badParallelWriteObject.code}`,
    );
    assert(
        "parallel_reports writeFiles object error is specific",
        badParallelWriteObject.stderr.includes(
            "parallel_reports groups must be read-only",
        ),
    ...
    assert(
        "parallel_reports terminal object exits non-zero",
        badParallelTerminalObject.code !== 0,
        `code=${badParallelTerminalObject.code}`,
    );
    assert(
        "parallel_reports terminal object error is specific",
        badParallelTerminalObject.stderr.includes(
            "parallel_reports groups must be read-only",
        ),
    ...
    assert(
        "parallel_reports allowUnknownClientRequests exits non-zero",
        badParallelUnknownObject.code !== 0,
        `code=${badParallelUnknownObject.code}`,
    );
    assert(
        "parallel_reports allowUnknownClientRequests error is specific",
        badParallelUnknownObject.stderr.includes(
            "parallel_reports groups must be read-only",
        ),
    ...
    assert(
        "write-capable parallel_reports reviewer exits non-zero",
        badParallelReviewerPolicy.code !== 0,
        `code=${badParallelReviewerPolicy.code}`,
    );
    assert(
        "write-capable parallel_reports reviewer error is specific",
        badParallelReviewerPolicy.stderr.includes(
            "parallel_reports groups must be read-only",
        ),
    ...
    assert(
        "parallel_reports reviewer allowUnknownClientRequests exits non-zero",
        badParallelReviewerUnknown.code !== 0,
        `code=${badParallelReviewerUnknown.code}`,
    );
    assert(
        "parallel_reports reviewer allowUnknownClientRequests error is specific",
        badParallelReviewerUnknown.stderr.includes(
            "parallel_reports groups must be read-only",
        ),
    ...
    assert(
        "parallel_reports writer field exits non-zero",
        badParallelWriterField.code !== 0,
        `code=${badParallelWriterField.code}`,
    );
    assert(
        "parallel_reports writer field error is specific",
        badParallelWriterField.stderr.includes(
            "writer and writerPermissions are only valid for single_writer groups",
        ),
    ...
    assert(
        "parallel_reports writerPermissions exits non-zero",
        badParallelWriterPermissions.code !== 0,
        `code=${badParallelWriterPermissions.code}`,
    );
    assert(
        "parallel_reports writerPermissions error is specific",
        badParallelWriterPermissions.stderr.includes(
            "writer and writerPermissions are only valid for single_writer groups",
        ),
    ...
    assert(
        "write-capable single_writer reviewer exits non-zero",
        badSingleWriterReviewer.code !== 0,
        `code=${badSingleWriterReviewer.code}`,
    );
    assert(
        "write-capable single_writer reviewer error is specific",
        badSingleWriterReviewer.stderr.includes(
            "reviewerPermissions must be read-only",
        ),
    ...
    assert(
        "terminal-capable single_writer reviewer exits non-zero",
        badSingleWriterReviewerTerminal.code !== 0,
        `code=${badSingleWriterReviewerTerminal.code}`,
    );
    assert(
        "terminal-capable single_writer reviewer error is specific",
        badSingleWriterReviewerTerminal.stderr.includes(
            "reviewerPermissions must be read-only",
        ),
    ...
    assert(
        "single_writer reviewer allowUnknownClientRequests exits non-zero",
        badSingleWriterReviewerUnknown.code !== 0,
        `code=${badSingleWriterReviewerUnknown.code}`,
    );
    assert(
        "single_writer reviewer allowUnknownClientRequests error is specific",
        badSingleWriterReviewerUnknown.stderr.includes(
            "reviewerPermissions must be read-only",
        ),
    ...
    assert(
        "unknown attachApprovedPlanFrom exits non-zero",
        unknownPlanSource.code !== 0,
        `code=${unknownPlanSource.code}`,
    );
    assert(
        "unknown attachApprovedPlanFrom error is specific",
        unknownPlanSource.stderr.includes(
            "attachApprovedPlanFrom references unknown agent group",
        ),
    ...
    assert(
        "single_writer writerPermissions remains valid",
        goodSingleWriterWithWriterPermissions.code === 0,
        goodSingleWriterWithWriterPermissions.stderr,
    );
    ...
    assert(
        "single_writer writerPermissions allowUnknownClientRequests remains valid",
        goodSingleWriterAllowUnknown.code === 0,
        goodSingleWriterAllowUnknown.stderr,
    );
    ...
    assert(
        "absolute artifactDir exits non-zero",
        badRootArtifactAbsolute.code !== 0,
        `code=${badRootArtifactAbsolute.code}`,
    );
    assert(
        "absolute artifactDir message is specific",
        badRootArtifactAbsolute.stderr.includes(
            'artifactDir must be workspace-relative, got absolute path "/tmp/outside"',
        ),
    ...
    assert(
        "traversal artifactDir exits non-zero",
        badRootArtifactTraversal.code !== 0,
        `code=${badRootArtifactTraversal.code}`,
    );
    assert(
        "traversal artifactDir message is specific",
        badRootArtifactTraversal.stderr.includes(
            'artifactDir must not contain ".." path segments',
        ),
    ...
    assert(
        "absolute group artifactDir exits non-zero",
        badGroupArtifactAbsolute.code !== 0,
        `code=${badGroupArtifactAbsolute.code}`,
    );
    assert(
        "absolute group artifactDir message is specific",
        badGroupArtifactAbsolute.stderr.includes(
            'agentGroups.plan.artifactDir must be workspace-relative, got absolute path "/tmp/outside"',
        ),
    ...
    assert(
        "traversal group artifactDir exits non-zero",
        badGroupArtifactTraversal.code !== 0,
        `code=${badGroupArtifactTraversal.code}`,
    );
    assert(
        "traversal group artifactDir message is specific",
        badGroupArtifactTraversal.stderr.includes(
            'agentGroups.plan.artifactDir must not contain ".." path segments',
        ),
...
`,
    ...
    writeStubScript(
        sub0,
        "function hello(x) { if (!x) return null; return x; }",
    );
    fs.writeFileSync(
        cfgPath,
        JSON.stringify({
            maxTurns: 3,
            subAgents: [
                { name: "StubA", command: "node", args: [sub0], env: {} },
            ],
            reviewer: {
                name: "StubR",
                command: "node",
    ...
    assert("stopReason is end_turn", result?.stopReason === "end_turn");
    assert(
        "final text contains synthesis",
        result?.content?.[0]?.text?.includes("async"),
    );
    const roundTexts = notifs
        .filter((n) =>
            n.params?.update?.content?.text?.startsWith("\n---\n## Round"),
        )
        .map((n) => n.params.update.content.text);
    assert(
        "2 rounds executed",
        roundTexts.length === 2,
        `got ${roundTexts.length}`,
    );
...
async function test_max_turns_respected() {
    ...
    assert(
        "mentions MAX_TURNS",
        result?.content?.[0]?.text?.includes("MAX_TURNS"),
    );
    const rounds = notifs.filter((n) =>
        n.params?.update?.content?.text?.startsWith("\n---\n## Round"),
    );
    assert(
        "exactly 2 rounds (MAX_TURNS)",
        rounds.length === 2,
        `got ${rounds.length}`,
    );
...
async function test_agent_group_workflow_advances_after_approval() {
    ...
    const streamed = notifs
        .map((n) => n.params?.update?.content?.text || "")
        .join("\n");

    assert(
        "first prompt used plan reviewer",
        first.includes("plan-approved"),
        first,
    );
    assert(
        "second prompt used code reviewer",
        second.includes("code-approved"),
        second,
    );
    ...
    assert(
        "advance notice was streamed",
        streamed.includes('Next prompt will use group "code"'),
    );
...
async function test_agent_group_prompt_directive_selects_group() {
    ...
    assert(
        "directive prompt used code reviewer",
        text.includes("code-approved"),
        text,
    );
    assert("streamed code group status", streamed.includes("Group: code"));
    assert(
        "did not stream plan group status",
        !streamed.includes("Group: plan"),
        streamed,
    );
...
async function test_retry_on_429() {
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            agentTimeoutMs: 5000,
            subAgents: [
                { name: "StubAgent", command: "node", args: [sub0], env: {} },
            ],
            reviewer: {
                name: "StubReviewer",
                command: "node",
    ...
    assert(
        "result contains synthesis",
        result?.content?.[0]?.text?.includes("42"),
    );
    const retryNotifs = notifs.filter((n) =>
        n.params?.update?.content?.text?.includes("Retrying"),
    );
    assert(
        "retry notifications emitted",
        retryNotifs.length >= 2,
        `got ${retryNotifs.length}`,
    );
...
`,
    ...
    assert("stopReason is end_turn", result?.stopReason === "end_turn");
    assert(
        "final result is APPROVED synthesis",
        result?.content?.[0]?.text?.includes("world"),
    );
    const failNotifs = notifs.filter((n) =>
        n.params?.update?.content?.text?.includes("FAILED"),
    );
    assert(
        "bad agent failure surfaced in stream",
        failNotifs.length >= 1,
        `got ${failNotifs.length}`,
    );
...
`;
    const sub0 = path.join(tmpDir, "sub0.js");
    fs.writeFileSync(sub0, badAgent);
    const sub1 = path.join(tmpDir, "sub1.js");
    fs.writeFileSync(sub1, badAgent);
...
async function test_rate_limiting() {
    ...
    assert(
        "both agents ran",
        result?.content?.[0]?.text?.includes("both results"),
    );
    // Second agent had to wait ≥ ~900ms for its token (allow some slack for CI)
    assert(
        `total time > 900ms due to rate limit (got ${elapsed}ms)`,
        elapsed >= 900,
        `${elapsed}ms`,
    );
    const waitNotifs = notifs.filter((n) =>
        n.params?.update?.content?.text?.includes("rate limit"),
    );
    assert(
        "rate-limit wait notification emitted",
        waitNotifs.length >= 1,
        `got ${waitNotifs.length}`,
    );
    assert(
        "rate-limit wait notifications are throttled",
        waitNotifs.length <= 3,
        `got ${waitNotifs.length}`,
    );
...
`,
    ...
    assert("stopReason is end_turn", result?.stopReason === "end_turn");
    assert(
        "both explicitly-keyed agents started without command-bucket serialization",
        spread < 700,
        `start spread ${spread}ms`,
    );
    assert(
        "no rate-limit wait notification emitted for distinct explicit buckets",
        waitNotifs.length === 0,
        `got ${waitNotifs.length}`,
    );
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            subAgents: [
                { name: "SlowAgent", command: "node", args: [slow], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    assert("stopReason is end_turn", result?.stopReason === "end_turn");
    const beats = notifs.filter((n) =>
        n.params?.update?.content?.text?.includes("still working"),
    );
    assert(
        "heartbeat fired at least twice",
        beats.length >= 2,
        `got ${beats.length}`,
    );
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            envIsolation: true,
            subAgents: [
                { name: "Probe", command: "node", args: [probe], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    const env = {
        ...process.env,
        TEST_FAKE_SECRET: "leaky-token-do-not-share",
    };
    const proc = spawn("node", [path.join(__dirname, "..", "index.js")], {
    ...
    const text = result?.content?.[0]?.text || "";
    assert(
        "isolated agent does NOT see TEST_FAKE_SECRET",
        !text.includes("leaky-token-do-not-share"),
        `leaked: ${text}`,
    );
...
`,
                ...
                subAgents: [
                    {
                        ...
                        env: {
                            ORCH_PLACEHOLDER_VALUE:
                                "{env:ORCH_TEST_DOTENV_VALUE}",
                            ORCH_BARE_VALUE: "$ORCH_TEST_DOTENV_BARE",
                            ORCH_JSON_VALUE:
                                '{"$schema":"kept","value":"${ORCH_TEST_DOTENV_VALUE}"}',
                        },
                    },
                ],
                reviewer: {
        ...
        const text = result?.content?.[0]?.text || "";
        assert(
            "{env:VAR} placeholders are expanded from sibling .env",
            text.includes("dotenv-env-ok"),
            text,
        );
    } finally {
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            subAgents: [
                { name: "Flooder", command: "node", args: [flooder], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    assert(
        ...
        result?.content?.[0]?.text?.includes("All sub-agents failed") ||
            notifs.some((n) =>
                n.params?.update?.content?.text?.includes("exceeded"),
            ),
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            agentTimeoutMs: 5000,
            subAgents: [
                { name: "Injector", command: "node", args: [sub0], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    assert(
        "injection neutralized (sanitized-ok)",
        text.includes("sanitized-ok"),
        `got: ${text}`,
    );
    assert(
        "does NOT contain injection-succeeded",
        !text.includes("injection-succeeded"),
    );
...
async function test_stdin_close_shuts_down() {
    ...
    const exited = await Promise.race([
        new Promise((r) =>
            proc.on("exit", (code) => r({ code, timedOut: false })),
        ),
        new Promise((r) =>
            setTimeout(() => {
                proc.kill("SIGKILL");
                r({ code: null, timedOut: true });
            }, 3000),
        ),
    ]);

    assert(
        "orchestrator exited after stdin close",
        !exited.timedOut,
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            subAgents: [
                { name: "SlowSub", command: "node", args: [slowSub], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    proc.on("exit", () => {
        for (const { reject } of pending.values())
            reject(new Error("orchestrator exited"));
        pending.clear();
    });
    ...
    try {
        await rpc("initialize", {
            protocolVersion: 1,
            clientInfo: { name: "stdin-close-kill-test" },
        });
        ...
        assert(
            "orchestrator exited after stdin close during in-flight prompt",
            !exited.timedOut,
            `stderr=${stderr.join("")}`,
        );
        ...
        const gone =
            ...
            (await waitForProcessGone(childPid, 3000));
        assert(
            "SIGTERM-ignoring child PID is gone after orchestrator shutdown",
            gone,
...
async function test_approved_parsing_tolerant() {
    ...
    assert(
        "synthesis extracted",
        result?.content?.[0]?.text?.includes("synthesis here"),
    );
    assert(
        "did not loop",
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            agentTimeoutMs: 5000,
            subAgents: [
                { name: "BlockEcho", command: "node", args: [sub0], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    const text = result?.content?.[0]?.text || "";
    assert(
        "sub-agent received both blocks (text+image)",
        text.includes("received-blocks:text,text,image"),
        `got: ${text.slice(0, 300)}`,
    );
    assert(
        "reviewer received text prompt plus image block",
        text.includes("reviewer-blocks:text,image"),
        `got: ${text.slice(0, 300)}`,
    );
...
`,
    ...
    const text = results[0]?.content?.[0]?.text || "";
    assert(
        "follow-up round received original text, image, and questions",
        text.includes("round2-rich-ok"),
        `got: ${text.slice(0, 300)}`,
    );
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            agentTimeoutMs: 30000,
            subAgents: [
                { name: "ManyLines", command: "node", args: [sub0], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    assert(
        ...
        result?.content?.[0]?.text?.includes("all chunks received"),
    );
    assert(
        `processed ${LINE_COUNT} lines in under 10s`,
        elapsed < 10000,
        `${elapsed}ms`,
    );
...
`,
    ...
    const result = await new Promise((resolve, reject) => {
        ...
        const startupTimer = setTimeout(
            () =>
                startupReject(
                    new Error(
                        "Timed out waiting for cancellation startup stream",
                    ),
                ),
            5000,
        );
        const maybeResolveStartup = () => {
            ...
            if (
                streamed.includes("Round 1 / 1") &&
                streamed.includes("Running 1 sub-agent")
            ) {
    ...
    const streamed = notifs
        ...
        .join("\n");
    assert(
        "cancelled prompt reports cancelled stopReason",
        result?.stopReason === "cancelled",
    );
    assert(
        "cancelled prompt returns cancellation message",
        result?.content?.[0]?.text?.includes("Cancelled by user"),
        JSON.stringify(result),
    );
    assert(
        "cancelled prompt still streamed round startup state",
        streamed.includes("Round 1 / 1") &&
            streamed.includes("Running 1 sub-agent"),
        streamed,
    );
    assert(
        "cancelled prompt did not invoke reviewer",
        !fs.existsSync(reviewerFlag),
        fs.existsSync(reviewerFlag)
            ? fs.readFileSync(reviewerFlag, "utf8")
            : "",
    );
...
async function test_unknown_group_directive_returns_rpc_error() {
    ...
    try {
        await runOrchestratorPrompts(cfgPath, tmpDir, [
            [
                {
                    type: "text",
                    text: "@orchestrator group: missing\nTry this anyway.",
                },
            ],
        ]);
    } catch (caught) {
    ...
    assert(
        "unknown group uses invalid params error code",
        err?.code === -32602,
        String(err?.code),
    );
    assert(
        "unknown group error mentions the missing group",
...
`,
    ...
    assert(
        "streaming overflow aborts the prompt path",
        text.includes("All sub-agents failed"),
        text,
    );
    assert(
        "streaming overflow uses a specific maxOutputBytes error",
...
async function test_max_output_bytes_aborts_direct_response_agent() {
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            maxOutputBytes: 1024,
            subAgents: [
                { name: "DirectSub", command: "node", args: [sub0], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    assert(
        "direct overflow aborts the prompt path",
        text.includes("All sub-agents failed"),
        text,
    );
    assert(
        "direct overflow uses a specific maxOutputBytes error",
...
`,
    ...
    assert(
        "direct response at maxOutputBytes succeeds",
        text.includes("direct-limit-ok") &&
            !text.includes("All sub-agents failed"),
        text,
    );
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            agentTimeoutMs: 5000,
            subAgents: [
                { name: "GoodSub", command: "node", args: [sub0], env: {} },
            ],
            reviewer: {
                name: "BadReviewer",
                command: "node",
    ...
    assert(
        "reviewer failure returns the documented fallback message",
        text.includes("Reviewer could not complete"),
        text,
    );
    assert(
        "reviewer failure includes the best available sub-agent result",
...
rl.on('line',line=>{
  ...
  if(msg.method==='initialize') send({jsonrpc:'2.0',id:msg.id,result:{protocolVersion:1,agentInfo:{name:'r',version:'1'},agentCapabilities:{}}});
    ...
    const text = result?.content?.[0]?.text || "";
    assert(
        "sub-agent result with missing newline was processed",
        text.includes("should-process-payload") &&
            !text.includes("All sub-agents failed"),
        text,
    );
    fs.rmSync(tmpDir, { recursive: true, force: true });
  ...
  if(msg.method==='initialize') send({jsonrpc:'2.0',id:msg.id,result:{protocolVersion:1,agentInfo:{name:'r',version:'1'},agentCapabilities:{}}});
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            agentTimeoutMs: 5000,
            subAgents: [
                { name: "DriftAgent", command: "node", args: [sub0], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    assert("stopReason is end_turn", result?.stopReason === "end_turn");
    assert(
        "finished in reasonable time",
        elapsed >= 1900 && elapsed < 5000,
        "elapsed: " + elapsed,
    );
    fs.rmSync(tmpDir, { recursive: true, force: true });
}
...
`,
    ...
    const text = result?.content?.[0]?.text || "";
    assert(
        "child write capability was masked and write request denied",
        text.includes("write-denied"),
        text,
    );
    assert(
        "denied write was not proxied to Zed",
...
`,
    ...
    const launched = fs.existsSync(launchedFile)
        ? fs.readFileSync(launchedFile, "utf8")
        : "";
    const text = result?.content?.[0]?.text || "";
    assert("writer result was approved", text.includes("writer-ok"), text);
    assert(
        "only configured writer was launched",
        launched.trim() === "writer",
        launched,
    );
    assert(
        "writer write request was proxied to Zed",
        zedWrites.length === 1,
        `got ${zedWrites.length}`,
    );
...
async function test_plan_artifacts_are_written() {
    ...
    const reviewerPrompt = files.find((f) =>
        /round-001\/reviewer-prompt\.md$/.test(f),
    );

    assert(
        "manifest, prompt, report, reviewer prompt, reviewer, and approved plan files exist",
        rel.some((f) => f.endsWith("manifest.json")) &&
            !!inputPrompt &&
            !!report &&
            !!reviewerPrompt &&
            !!reviewer &&
    ...
    assert(
        "approved plan contains synthesis",
    ...
    assert(
        "reviewer prompt artifact contains task context",
        fs
            .readFileSync(reviewerPrompt, "utf8")
            .includes("Write a hello-world function."),
...
`,
    ...
    const result = await new Promise((resolve, reject) => {
        ...
        (async () => {
            try {
                await rpc("initialize", {
                    protocolVersion: 1,
                    clientInfo: { name: "cancel-notification-test" },
                });
                const { sessionId } = await rpc("session/new", { cwd: tmpDir });
                const promptPromise = rpc("session/prompt", {
                    sessionId,
    ...
    assert(
        "cancel notification emitted no id-less response frame",
...
`,
    ...
    assert(
        "child was escalated to SIGKILL",
        signal === "SIGKILL",
        `signal=${signal}`,
    );
...
async function test_redact_scrubs_secrets() {
    ...
    assert(
        "sk- key redacted",
        !redact("Error: sk-ant-api03-ABCDEFGHIJKLMNOP12345678").includes(
            "ABCDEF",
        ),
    );
    assert(
        "Bearer token redacted",
        !redact("headers: Bearer eyJhbGciOiJIUzI1NiJ9.payload.sig").includes(
            "eyJhbG",
        ),
    );
    assert(
        "AIza key redacted",
        !redact("AIzaSyABCDEFGHIJKLMNOPQRSTUVWXYZ01234567890").includes(
            "ABCDEF",
        ),
    );
    assert(
        "generic api_key= redacted",
    ...
    assert(
        "quoted JSON api_key redacted",
        redact('"api_key":"abcdefghijklmnopqrstuvwxyz"') ===
            '"api_key":"…REDACTED…"',
    );
    assert(
        "spaced quoted JSON api_key redacted",
        redact('"api_key": "abcdefghijklmnopqrstuvwxyz"') ===
            '"api_key": "…REDACTED…"',
    );
    assert(
        "single-quoted token redacted",
        redact("'token': 'abcdefghijklmnopqrstuvwxyz'") ===
            "'token': '…REDACTED…'",
    );
    assert(
        "double-quoted token redacted",
        redact('token: "abcdefghijklmnopqrstuvwxyz"') === 'token: "…REDACTED…"',
    );
    assert(
        "quoted password assignment redacted",
        redact('password = "correct-horse-battery-staple"') ===
            'password = "…REDACTED…"',
    );
    assert(
        "single-quoted credential redacted",
...
async function test_semaphore_limits_concurrency() {
    ...
    assert(
        "semaphore never exceeded limit of 2",
        maxObserved <= 2,
...
`,
    ...
    const streamed = notifs
        .map((n) => n.params?.update?.content?.text || "")
        .join("");
    const commandsUpdate = notifs.find(
        (n) => n.params?.update?.sessionUpdate === "available_commands_update",
    );
    const commandNames = (
        commandsUpdate?.params?.update?.availableCommands || []
    ).map((c) => c.name);

    assert(
        "orchestrator advertises /plan to ACP clients",
        commandNames.includes("plan"),
        JSON.stringify(commandNames),
    );
    assert(
        "slash /plan shortcut result approved",
        streamed.includes("slash-shortcut-ok") ||
            result.stopReason === "end_turn",
    );
    {
        const { requestedGroupName, stripGroupDirective } = require(
            path.join(__dirname, "..", "orchestrator"),
        );
        const task = [{ type: "text", text: "/plan Do analysis." }];
        const stripped = stripGroupDirective(task, ["plan"]);
        assert(
            "/plan with same-line input selects plan group",
            requestedGroupName(task, ["plan"]) === "plan",
        );
        assert(
            "/plan with same-line input preserves input text",
            stripped[0]?.text === "Do analysis.",
    ...
    const slashResult = await new Promise((resolve, reject) => {
        ...
        const rpc = (m) =>
            new Promise((res, rej) => {
                ...
                pending.set(msgId, { resolve: res, reject: rej });
                proc2.stdin.write(
                    JSON.stringify({ jsonrpc: "2.0", id: msgId, ...m }) + "\n",
                );
            });
        ...
        (async () => {
            try {
                await rpc({
                    method: "initialize",
                    params: { protocolVersion: 1, clientInfo: { name: "t" } },
                });
    ...
    assert(
        "/plan shortcut prompt completes",
        slashResult?.stopReason === "end_turn",
    );
...
`,
    ...
    assert(
        "agent HOME was NOT the real user HOME",
        !streamed.includes(`HOME=${process.env.HOME}\n`),
    );
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            agentTimeoutMs: 5000,
            subAgents: [
                { name: "OldAgent", command: "node", args: [subPath], env: {} },
            ],
            reviewer: {
                name: "Rev",
                command: "node",
    ...
    assert(
        "version mismatch is fatal: orchestrator still responds to Zed gracefully",
        result?.stopReason === "end_turn",
    );
    assert(
        "version mismatch surfaces mismatch error in streamed output",
        streamed.includes("protocolVersion mismatch") ||
            streamed.includes("All sub-agents failed"),
...
`,
    ...
    assert(
        "token telemetry line appears in stream",
        streamed.includes("tokens:"),
        streamed,
    );
...
`,
    ...
    assert(
        "orchestrator completed despite dead agent",
        result?.stopReason === "end_turn",
    );
    assert(
        "probe failure is reported in stream",
        streamed.includes("probe failed") || streamed.includes("skipped"),
    );
...
`,
    ...
    assert(
        "prompt completed with rate-limited probes",
        result?.stopReason === "end_turn",
    );
    assert(
        "first two probe initializes were serialized by rate limit",
        probeSpread >= 900,
        `spread=${probeSpread}ms; times=${initTimes.join(",")}`,
    );
    assert(
        "probe rate-limit notification emitted",
        streamed.includes("probe rate limit"),
        streamed,
    );
...
`,
    ...
    const timestamps = fs
        ...
        .map(Number);
    assert(
        "all 5 agents reported a start time",
        timestamps.length === 5,
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            agentTimeoutMs: 5000,
            subAgents: [
                { name: "Sub", command: "node", args: [subPath], env: {} },
            ],
            reviewer: {
                name: "Rev",
                command: "node",
    ...
    const promptLines = fs
        .readFileSync(reviewerFile, "utf8")
        .split("\n")
        .filter(Boolean)
        .map((l) => {
            try {
    ...
    assert(
        ...
        `got ${promptLines.length}`,
    );
    const round2Text = promptLines[1]?.[0]?.text || "";
    assert(
        "round 2 reviewer prompt includes full unchanged agent result",
        round2Text.includes("round 1 answer") &&
            !round2Text.includes("*(unchanged)*"),
...
`,
    ...
    assert(
        "secret key not in streamed output",
        !streamed.includes("SUPERSECRETKEY"),
        streamed.slice(0, 500),
    );
    assert(
        "redacted placeholder present or error present",
        streamed.includes("FAILED") || streamed.includes("REDACTED"),
    );
...
async function test_default_workflow_does_not_auto_advance_to_code() {
    ...
    assert(
        "first prompt used plan group",
        first.includes("plan-default-ok"),
        first,
    );
    assert(
        "second prompt still used plan group",
        second.includes("plan-default-ok"),
        second,
    );
    assert(
        "default workflow did not advance to code",
        !streamed.includes("Group: code"),
        streamed,
    );
...
`,
    ...
    const text = result?.content?.[0]?.text || "";
    assert(
        "/login reached the child agent",
        text.includes("login-preserved"),
        text,
    );
...
async function test_single_writer_requires_exact_writer_match() {
    ...
    assert(
        "missing writer exits non-zero",
        missingWriter.code !== 0,
        `code=${missingWriter.code}`,
    );
    assert(
        "missing writer error is specific",
        missingWriter.stderr.includes("writer is required"),
        missingWriter.stderr,
    );
    ...
    assert(
        "unknown writer exits non-zero",
        wrongWriter.code !== 0,
        `code=${wrongWriter.code}`,
    );
    assert(
        "unknown writer error is specific",
        wrongWriter.stderr.includes("must match exactly one"),
        wrongWriter.stderr,
    );
}
...
`,
    ...
    const text = result?.content?.[0]?.text || "";
    assert(
        "reviewer write request was denied",
        text.includes("reviewer-read-only"),
        text,
    );
    assert(
        "reviewer write was not proxied to Zed",
        zedWrites.length === 0,
        `got ${zedWrites.length}`,
    );
...
`,
    ...
    const streamed = notifs
        ...
        .join("");
    assert(
        "writer prompt ran only once after side effect",
        attempts === 1,
        `attempts=${attempts}`,
    );
    assert(
        "no retry notification emitted after side effect",
        !streamed.includes("Retrying"),
        streamed,
    );
...
`,
    ...
    const runDirs = fs
        .readdirSync(path.join(tmpDir, ".plan", "orchestrator"))
        .filter((name) => name.startsWith("orch-"));

    assert(
        "code writer received plan-group approved plan only",
        codeText.includes("code-review-ok"),
        codeText,
    );
    assert(
        "each prompt wrote a distinct artifact run directory",
        runDirs.length === 3,
        runDirs.join(", "),
    );
...
`,
    ...
    const { results, notifs } = await runOrchestratorPrompts(
        cfgPath,
        tmpDir,
        [
            [{ type: "text", text: "/plan\nCreate plan." }],
        ...
        ],
        {
            afterPrompt: async (index) => {
                ...
                const runDir = fs
                    .readdirSync(root)
                    .find((name) => name.startsWith("orch-"));
    ...
    const streamed = streamedText(notifs);
    assert(
        "tampered plan was not attached to code prompt",
        codeText.includes("hash-check-ok"),
        codeText + streamed,
    );
    assert(
        "hash mismatch warning was streamed",
        streamed.includes("hash mismatch"),
        streamed,
    );
...
async function test_successful_outputs_are_redacted_in_stream_and_artifacts() {
    ...
    assert(
        "secret not streamed to Zed",
        !streamed.includes("SUPERSECRETKEY"),
        streamed,
    );
    assert(
        "secret not written to artifacts",
        !artifactText.includes("SUPERSECRETKEY"),
        artifactText,
    );
    assert(
        "quoted secret not streamed to Zed",
        !streamed.includes("QUOTEDSECRETKEY"),
        streamed,
    );
    assert(
        "quoted secret not written to artifacts",
        !artifactText.includes("QUOTEDSECRETKEY"),
        artifactText,
    );
    assert(
        "redaction marker appears in stream or artifacts",
...
async function test_artifact_store_rejects_symlink_escape() {
    ...
    const { writeArtifact } = require(
        path.join(__dirname, "..", "artifact-store"),
    );
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "orch-test-"));
    ...
    assert(
        "outside target was not written",
        !fs.existsSync(path.join(outside, "escape.md")),
    );
    assert(
        "no subdirectory was created outside workspace before rejection",
...
async function test_policy_preserves_resource_link_blocks() {
    ...
    const { filterPromptContentForCapabilities } = require(
        path.join(__dirname, "..", "policy"),
    );
    ...
    assert(
        "resource_link survives without advertised prompt capabilities",
        types === "text,resource_link",
        types,
    );
    assert(
        "image and embedded resource were dropped",
        filtered.dropped.includes("image") &&
            filtered.dropped.includes("resource"),
...
async function test_cred_home_cannot_be_overridden_by_env_or_passenv() {
    ...
    assert(
        "CODEX_HOME is forced under credHome",
        env.CODEX_HOME === path.join(credHome, "codex"),
        env.CODEX_HOME,
    );
    assert(
...
`,
    ...
    const streamed = notifs
        ...
        .join("");
    assert(
        "prompt still completes at orchestrator level",
        result?.stopReason === "end_turn",
    );
    assert(
        "max_tokens is surfaced as an agent failure",
        streamed.includes("Agent stopped with max_tokens") ||
...
async function test_single_writer_group_is_one_shot() {
    ...
    const streamed = notifs
        ...
        .join("\n");
    assert(
        "/code prompt used code group",
        first.includes("code-one-shot"),
        first,
    );
    assert(
        "next prompt reset to default plan group",
        second.includes("plan-after-code"),
        second,
    );
    assert(
        "both code and plan group statuses streamed",
        streamed.includes("Group: code") && streamed.includes("Group: plan"),
        streamed,
    );
...
async function test_code_reviewer_receives_attached_approved_plan() {
    ...
    writeStubScript(
        planReviewer,
        "APPROVED: plan-approved-content-for-reviewer",
    );
    writeStubScript(writerPath, "writer-result");
    fs.writeFileSync(
        codeReviewer,
...
`,
    ...
    const { results } = await runOrchestratorPrompts(cfgPath, tmpDir, [
        ...
        [
            {
                type: "text",
                text: "@orchestrator group: implement\nImplement approved plan.",
            },
        ],
    ]);
    const text = results[1]?.content?.[0]?.text || "";
    assert(
        "code reviewer saw approved plan text",
        text.includes("reviewer-saw-plan"),
        text,
    );
...
async function test_tilde_cred_home_is_accepted() {
    ...
    assert(
        "tilde credHome resolves under user home",
        env.HOME === expected,
        `${env.HOME} !== ${expected}`,
    );
    assert(
        "tilde credHome directory exists",
...
async function test_group_directive_must_be_first_non_empty_line() {
    ...
    const { results, notifs } = await runOrchestratorPrompts(cfgPath, tmpDir, [
        [
            {
                type: "text",
                text: "Please review this pasted content:\n\n/code\n",
            },
        ],
        [{ type: "text", text: "/code\nNow explicitly code." }],
    ]);
    ...
    assert(
        "pasted /code did not switch to code group",
        first.includes("plan-directive-safe") &&
            streamed.includes("Group: plan"),
        first + streamed,
    );
    assert(
        "first-line /code still switches to code group",
        second.includes("code-directive-used"),
        second,
    );
...
`,
    ...
    const text = result?.content?.[0]?.text || "";
    assert(
        "planning phase instruction reached child agent",
        text.includes("phase-instruction-ok"),
        text,
    );
...
async function test_workflow_auto_advances_to_read_only_review_group() {
    ...
    assert(
        "next prompt auto-used review group",
        second.includes("review-approved") &&
            streamed.includes("Group: review"),
        second + streamed,
    );
    assert(
        "review group reset back to plan after one prompt",
        third.includes("plan-approved"),
        third,
    );
...
async function test_workflow_does_not_auto_advance_to_write_capable_group() {
    ...
    assert(
        "first prompt approved in plan group",
        first.includes("plan-still-active"),
        first,
    );
    assert(
        "write-capable workflow target was not auto-selected",
        second.includes("plan-still-active") &&
            !second.includes("write-group-ran"),
        second,
    );
    assert(
        "user is told to invoke write-capable group explicitly",
        streamed.includes("Use /deploy explicitly") &&
            !streamed.includes("Group: deploy"),
        streamed,
    );
...
async function test_policy_denies_unknown_fs_and_destructive_permission_kinds() {
    ...
    const { assertAllowedClientRequest } = require(
        path.join(__dirname, "..", "policy"),
    );
    const readOnly = {
        readFiles: true,
            ...
            assertAllowedClientRequest(
                {
                    ...
                    params: {
                        toolCall: { kind, title: kind, status: "pending" },
                    },
                },
                noWriteButTerminal,
                "TestAgent",
    ...
    assert(
        "delete and move permission requests require write policy",
        deniedKinds.join(",") === destructiveKinds.join(","),
        deniedKinds.join(","),
    );
}
...
`,
    ...
    const streamed = streamedText(notifs);
    assert(
        "unsupported HTTP MCP server was dropped and stdio server kept",
        text.includes("mcp-filter-ok"),
        text + streamed,
    );
    assert(
        "MCP drop warning was streamed",
        streamed.includes("http-tools"),
        streamed,
    );
...
async function test_mcp_filter_keeps_bare_stdio_and_gates_http_sse() {
    ...
    const withoutCaps = orch.filterMcpServersForCapabilities(servers, {
        mcpCapabilities: {},
    });
    const keptWithoutCaps = withoutCaps.kept.map((s) => s.name).join(",");
    const stdio = withoutCaps.kept.find((s) => s.name === "zed-stdio");
    assert(
        "bare and explicit stdio are kept without child MCP capabilities",
        keptWithoutCaps === "zed-stdio,config-stdio",
        keptWithoutCaps,
    );
    assert(
        "bare stdio is normalized to ACP stdio shape without type",
        stdio &&
            !Object.prototype.hasOwnProperty.call(stdio, "type") &&
            stdio.command === "/bin/echo" &&
            Array.isArray(stdio.args) &&
            Array.isArray(stdio.env),
        JSON.stringify(stdio),
    );
    assert(
        "HTTP and SSE are dropped without matching capabilities",
        withoutCaps.dropped.includes("http-tools") &&
            withoutCaps.dropped.includes("sse-tools"),
        withoutCaps.dropped.join(","),
    );

    const withCaps = orch.filterMcpServersForCapabilities(servers, {
        mcpCapabilities: { http: true, sse: true },
    });
    assert(
        "HTTP and SSE are kept when child capabilities advertise them",
        withCaps.kept.map((s) => s.name).join(",") ===
            "zed-stdio,config-stdio,http-tools,sse-tools",
...
`,
            ...
            mcpServers: [
                {
                    ...
                    name: "stdio-tools",
                    command: "node",
                    env: [{ name: "TOKEN", value: "${ORCH_TEST_MCP_TOKEN}" }],
                },
                {
                    type: "http",
                    name: "http-tools",
    ...
    const text = result?.content?.[0]?.text || "";
    assert(
        "stdio args/env and http headers were normalized before forwarding",
...
async function test_semaphore_cancellation_drains_queued_waiters() {
    ...
    assert(
        "all queued waiters settled after cancellation",
        settled.includes("second CANCELLED") &&
            settled.includes("third CANCELLED"),
        settled.join(","),
    );
    assert(
        "semaphore queue is empty after cancellation drain",
        sem.queued === 0,
        `queued=${sem.queued}`,
    );
}
...
`,
    ...
    const streamed = streamedText(notifs);
    assert(
        "read_only default sent zero MCP servers",
        text.includes("mcp-off-ok"),
        text,
    );
    assert(
        "all configured and session MCP servers were dropped",
        streamed.includes("http-tools") &&
            streamed.includes("stdio-tools") &&
            streamed.includes("zed-stdio"),
        streamed,
    );
...
`,
    ...
    const text = result?.content?.[0]?.text || "";
    assert(
        "code writer received Zed-style bare stdio MCP server",
        text.includes("zed-bare-stdio-ok"),
        text,
    );
...
async function test_unknown_client_method_denied_in_read_only() {
    ...
    const { assertAllowedClientRequest, normalizePolicy } = require(
        path.join(__dirname, "..", "policy"),
    );
    ...
    try {
        assertAllowedClientRequest(
            { method: "workspace/apply_patch", params: {} },
            readOnly,
            "TestAgent",
        );
    } catch (err) {
    ...
    assert(
        "denied error mentions method name",
        /workspace\/apply_patch/.test(deniedMessage),
        deniedMessage,
    );
    ...
    assert(
        "explicit writer escape hatch still allows unknown methods",
        explicitWriterAllowed,
    );
}
...
async function test_safe_env_omits_node_path_and_windows_home_keys() {
    ...
    assert(
        "NODE_PATH not in SAFE_ENV_KEYS",
        !SAFE_ENV_KEYS.includes("NODE_PATH"),
    );
    for (const winKey of ["USERPROFILE", "APPDATA", "LOCALAPPDATA"]) {
        assert(
            `${winKey} not in SAFE_ENV_KEYS (must require allowRealHome)`,
            !SAFE_ENV_KEYS.includes(winKey),
        );
        assert(
            `${winKey} listed in REAL_HOME_KEYS`,
            REAL_HOME_KEYS.includes(winKey),
        );
    }
    ...
    try {
        ...
        assert(
            "NODE_PATH not forwarded under envIsolation",
            isolated.NODE_PATH === undefined,
            `got: ${isolated.NODE_PATH}`,
        );
        for (const winKey of ["USERPROFILE", "APPDATA", "LOCALAPPDATA"]) {
            assert(
                `${winKey} not forwarded under envIsolation without allowRealHome`,
                isolated[winKey] === undefined,
                `got ${winKey}=${isolated[winKey]}`,
            );
        }

        const allowed = buildChildEnv(
            { envIsolation: true, allowRealHome: true },
            "/tmp",
        );
        for (const winKey of ["USERPROFILE", "APPDATA", "LOCALAPPDATA"]) {
            assert(
                `${winKey} forwarded when allowRealHome:true`,
                allowed[winKey] !== undefined,
                `${winKey}=${allowed[winKey]}`,
            );
        }

        const passEnvForwards = buildChildEnv(
            { envIsolation: true, passEnv: ["NODE_PATH"] },
            "/tmp",
        );
        assert(
            "NODE_PATH forwarded only when explicitly passed via passEnv",
            passEnvForwards.NODE_PATH === "/tmp/leaky-node-modules",
            passEnvForwards.NODE_PATH,
        );
    } finally {
...
`,
    ...
    assert(
        "agentTimeoutMs respected for initialize-hang (under 6s with 1.5s timeout)",
        elapsed < 6000,
        `elapsed=${elapsed}ms`,
    );
    assert(
        "initialize hang surfaced as TIMEOUT or all-failed",
        /TIMEOUT|All sub-agents failed/.test(streamed) ||
            /failed/i.test(result?.content?.[0]?.text || ""),
        streamed,
    );
...
`,
    ...
    const result = await new Promise((resolve, reject) => {
        ...
        (async () => {
            try {
                await rpc("initialize", {
                    protocolVersion: 1,
                    clientInfo: { name: "probe-cancel-test" },
                });
                const { sessionId } = await rpc("session/new", { cwd: tmpDir });
                const start = Date.now();
                const promptPromise = rpc("session/prompt", {
                ...
                const elapsed = Date.now() - start;
                resolve({
                    result: decoratePromptResult(promptResult, notifs),
                    elapsed,
                });
            } catch (err) {
                reject(err);
    ...
    assert(
        "cancelled probe returns cancelled stopReason",
        result.result?.stopReason === "cancelled",
        JSON.stringify(result.result),
    );
    assert(
        "cancelled probe completes well under probeTimeoutMs (<10s, not 30s)",
...
async function test_find_latest_approved_plan_surfaces_warnings() {
    ...
    if (typeof orch.findLatestApprovedPlan !== "function") {
        assert(
            "findLatestApprovedPlan exported for testability",
            false,
            "not exported",
        );
        return;
    ...
    const warnings = [];
    const file = orch.findLatestApprovedPlan(tmpDir, artifactDir, "plan", (w) =>
        warnings.push(w),
    );
    assert(
        "valid approved-plan still located despite a broken sibling",
        file === validPlan,
        `got ${file}, warnings=${warnings.join("|")}`,
    );
    assert(
        "warning emitted for unreadable approved-plan candidate",
...
async function test_find_latest_approved_plan_rejects_symlinked_orchestrator_root() {
    ...
    writeApprovedPlanFixture(
        path.join(externalRoot, "orch-evil", "approved-plan.md"),
        "plan",
        "external-approved-plan\n",
    );
    fs.mkdirSync(path.join(tmpDir, ".plan"), { recursive: true });
    fs.symlinkSync(
        externalRoot,
        path.join(tmpDir, ".plan", "orchestrator"),
        "dir",
    );

    const warnings = [];
    const file = orch.findLatestApprovedPlan(
        tmpDir,
        ".plan/orchestrator",
        "plan",
        (w) => warnings.push(w),
    ...
    assert(
        "symlinked .plan/orchestrator root is not accepted",
        file === null,
        `got ${file}`,
    );
    assert(
        "symlinked orchestrator root emits useful warning",
        warnings.some((w) =>
            w.includes("resolves outside workspace through symlink"),
        ),
        warnings.join("|"),
    );
...
async function test_find_latest_approved_plan_rejects_symlinked_plan_root() {
    ...
    assert(
        "symlinked .plan root is not accepted",
        file === null,
        `got ${file}`,
    );
    assert(
        "symlinked .plan root emits useful warning",
        warnings.some((w) =>
            w.includes("resolves outside workspace through symlink"),
        ),
        warnings.join("|"),
...
`,
    ...
    const streamed = notifs
        ...
        .join("");
    assert(
        "writer launched exactly once (no retry even without ACP side effect)",
        attempts === 1,
        `attempts=${attempts}`,
    );
    assert(
        "no retry notification emitted for single_writer",
        !streamed.includes("Retrying"),
        streamed,
    );
...
async function test_permission_objects_are_strict_without_schema_dependency() {
    ...
    const baseCodeGroup = (overrides) => ({
        ...
        agentGroups: {
            code: {
                ...
                writer: "Writer",
                subAgents: [
                    { name: "Writer", command: "node", args: [], env: {} },
                ],
                reviewer: {
                    name: "Reviewer",
                    command: "node",
    ...
    const stringFalse = await runInvalidConfig(
        baseCodeGroup({
            writerPermissions: {
                readFiles: true,
                writeFiles: "false",
    ...
    assert(
        "permission object with string boolean exits non-zero",
        stringFalse.code !== 0,
        `code=${stringFalse.code}`,
    );
    assert(
        "string boolean error points at writerPermissions.writeFiles",
        stringFalse.stderr.includes(
            "agentGroups.code.writerPermissions: permissions.writeFiles must be a boolean",
        ),
        stringFalse.stderr,
    );

    const typoKey = await runInvalidConfig(
        baseCodeGroup({
            writerPermissions: {
                readFiles: true,
                writeFile: false,
    ...
    assert(
        "permission object with typo key exits non-zero",
        typoKey.code !== 0,
        `code=${typoKey.code}`,
    );
    assert(
        "typo key error is explicit",
        typoKey.stderr.includes(
            'agentGroups.code.writerPermissions: permissions contains unknown key "writeFile"',
        ),
        typoKey.stderr,
    );
}
...
async function test_approved_plan_index_does_not_read_symlinked_index() {
    ...
    const { writeApprovedPlanIndex } = require(
        path.join(__dirname, "..", "artifact-store"),
    );

    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "orch-test-"));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), "orch-outside-"));
    fs.mkdirSync(path.join(tmpDir, ".plan", "orchestrator"), {
        recursive: true,
    });
    ...
    fs.writeFileSync(
        outsideIndex,
        JSON.stringify({
            secretFromOutside: "SHOULD_NOT_COPY",
            latestApprovedBySession: {},
        }),
    );
    fs.symlinkSync(
        outsideIndex,
        path.join(tmpDir, ".plan", "orchestrator", "index.json"),
    );
    ...
    assert(
        "symlinked index source was not copied into workspace artifact",
        !written.includes("SHOULD_NOT_COPY") &&
            !written.includes("secretFromOutside"),
        written,
    );
    assert(
        "new index still contains the current approved plan metadata",
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            agentTimeoutMs: 5000,
            subAgents: [
                { name: "NoisyAgent", command: "node", args: [sub], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    assert(
        "503 from stderr surfaced as provider HTTP notification",
        /provider HTTP 503/.test(text),
        text,
    );
    assert(
        "500 from stderr surfaced as provider HTTP notification",
        /provider HTTP 500/.test(text),
        text,
    );
    assert(
        "non-error stderr lines do not produce false positives",
        !/just a regular log line/.test(text),
        text,
    );
    ...
    assert(
        "repeated 503 stderr lines are throttled",
        count503 === 1,
        `count=${count503}`,
    );
...
`,
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            agentTimeoutMs: 5000,
            subAgents: [
                { name: "DebugSub", command: "node", args: [sub], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    assert("stopReason is end_turn", result?.stopReason === "end_turn");
    assert(
        "debug log activation banner present",
        /debug logging enabled/.test(stderr),
        stderr.slice(0, 400),
    );
    assert(
        "outgoing initialize frame logged",
        /\[debug\] \[DebugSub\] → .*method=initialize/.test(stderr),
        stderr.slice(0, 600),
    );
    assert(
        "incoming initialize result logged",
        /\[debug\] \[DebugSub\] ← .*result=/.test(stderr),
        stderr.slice(0, 600),
    );
    assert(
        "sub-agent stderr line mirrored to debug log",
        /\[debug\] \[DebugSub\] stderr: debug-test-stderr-marker-init/.test(
            stderr,
        ),
        stderr.slice(0, 600),
    );
    assert(
        "process spawn logged with pid",
        /\[debug\] \[DebugSub\] spawned pid=\d+/.test(stderr),
        stderr.slice(0, 600),
    );
    assert(
        "process exit logged",
        /\[debug\] \[DebugSub\] exited code=/.test(stderr),
        stderr.slice(0, 600),
    );
...
async function test_debug_log_disabled_by_default() {
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            ...
            agentTimeoutMs: 5000,
            subAgents: [
                { name: "QuietSub", command: "node", args: [sub], env: {} },
            ],
            reviewer: {
                name: "Reviewer",
                command: "node",
    ...
    assert("stopReason is end_turn", result?.stopReason === "end_turn");
    assert(
        "no [debug] lines without explicit opt-in",
        !/\[debug\]/.test(stderr),
        stderr.slice(0, 400),
    );
...
async function test_integration_real_claude_acp() {
    ...
    const check = spawnSync("claude-agent-acp", ["--version"], {
        stdio: "pipe",
    });
    if (check.status !== 0) {
        console.log(
            "  ⚠ claude-agent-acp not found — skipping integration test",
        );
        return;
    }
    ...
    assert(
        "[integration] real claude returned some text",
        streamed.length > 50,
        `got ${streamed.length} chars`,
    );

    fs.rmSync(tmpDir, { recursive: true, force: true });
}

async function test_session_resume() {
    console.log(
        "\n[Test 83] session/resume correctly restores from file persistence",
    );
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "orch-test-"));
            ...
            subAgents: [
                {
                    ...
                    args: [
                        "-e",
                        'console.log(`{"jsonrpc":"2.0","method":"session/new"}`); setInterval(()=>{},1000);',
                    ],
                    env: {},
                },
            ],
    ...
    const orch = require("child_process").spawn(
        "node",
    ...
    const rl = require("readline").createInterface({ input: orch.stdout });
    rl.on("line", (line) => {
        let msg;
        try {
    ...
    try {
        ...
        const { sessionId } = await rpc("session/new", { cwd: tmpDir });
        assert("session/new returns sessionId", sessionId);

        // Give the file persistence a moment
        await new Promise((r) => setTimeout(r, 50));
        ...
        assert("session file exists", fs.existsSync(sessionFile));

        const resumeRes = await rpc("session/resume", { sessionId });
        assert(
            "session/resume succeeds on valid id",
            resumeRes && !resumeRes.code,
        );
        ...
        const pathTraversal = await rpc("session/resume", {
        ...
        assert(
            "session/close removes in-memory state",
            closeRes && !closeRes.code,
        );
        assert(
            "session/close does not corrupt persisted state",
...
async function test_parse_reviewer_decision() {
    console.log(
        "\n[Test 84] parseReviewerDecision properly extracts text and rejects injection",
    );
    const { parseReviewerDecision } = require("../orchestrator.js");

    // Valid cases
    const r1 = parseReviewerDecision("APPROVED: final plan body");
    assert("starts with APPROVED:", r1.kind === "approved");
    assert("extracts body", r1.body === "final plan body");

    const r2 = parseReviewerDecision(
        "Thinking before final verdict...\nAPPROVED: final durable plan body",
    );
    assert("handles preambles", r2.kind === "approved");
    assert(
        "extracts body from preamble",
        r2.body === "final durable plan body",
    );

    const r3 = parseReviewerDecision("QUESTIONS: \n1. why?");
    assert("starts with QUESTIONS:", r3.kind === "questions");
    assert("extracts body", r3.body === "1. why?");

    // Invalid cases (regression)
    const r4 = parseReviewerDecision("APPROVED_BY_AGENT: child text only");
    assert("rejects sanitized APPROVED_BY_AGENT:", r4.kind === null);
}

async function test_reviewer_merges_chunks_and_direct_content() {
    console.log("\n[Test 85] Reviewer properly merges session/update chunks and direct response");
    
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "orch-test-"));
    const cfgPath = buildConfig(tmpDir, {
        subResponses: ["ok"],
        reviewerResponse: "APPROVED: should-not-run",
    });
...
rl.on("line", line => {
  ...
  if (msg.method === "initialize")
    ...
    if (msg.params?.prompt?.[0]?.text?.includes("Evaluate whether")) {
        send({jsonrpc:"2.0",method:"session/update",params:{update:{sessionUpdate:"agent_message_chunk",content:{type:"text",text:"Thinking before final verdict...\\n"}}}});
        setTimeout(() => {
            send({jsonrpc:"2.0",id:msg.id,result:{stopReason:"end_turn",content:[{type:"text",text:"APPROVED: final durable plan body"}]}});
        }, 10);
    } else {
        send({jsonrpc:"2.0",id:msg.id,result:{stopReason:"end_turn",content:[{type:"text",text:"task done"}]}});
    }
  }
  else if (msg.id != null)
    send({jsonrpc:"2.0",id:msg.id,result:{}});
...
`);
    ...
    walk(artifactRoot);
    const approved = files.find((f) => f.endsWith("approved-plan.md"));
    assert("approved-plan.md exists", !!approved);
    if (approved) {
        const approvedContent = fs.readFileSync(approved, "utf8");
        assert(
            "approved-plan.md contains final durable plan body",
            approvedContent.includes("final durable plan body")
        );
    }
}
...
(async () => {
    ...
    try {
        ...
        await T(
            test_single_writer_group_runs_only_writer_with_write_capability,
        );
        ...
        await T(
            test_find_latest_approved_plan_rejects_symlinked_orchestrator_root,
        );
        ...
        await T(test_session_resume);
        await T(test_parse_reviewer_decision);
        await T(test_reviewer_merges_chunks_and_direct_content);
