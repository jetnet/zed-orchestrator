...
try {
    ...
    process.stderr.write(
        `[orchestrator] Cannot read config: ${cfgPath}\n${e.message}\n`,
    );
    process.exit(1);
}
...
if (
    !cfg.agentGroups &&
    (!Array.isArray(cfg.subAgents) || !cfg.subAgents.length)
) {
    process.stderr.write(
        "[orchestrator] config.subAgents must be a non-empty array, or config.agentGroups must be provided\n",
    );
    process.exit(1);
}
    ...
    try {
        ...
        if (!validate(data)) {
            const msgs = (validate.errors || [])
                ...
                .join("\n");
            process.stderr.write(
                `[orchestrator] config schema errors:\n${msgs}\n`,
            );
            process.exit(1);
        }
    } catch (err) {
        if (err.code !== "MODULE_NOT_FOUND") {
            process.stderr.write(
                `[orchestrator] ajv validation error: ${err.message}\n`,
            );
        }
...
function resolveNameValueEntries(
    entries = [],
    { filterEmptyBearer = false } = {},
) {
    return entries
...
function resolveMcpServers(servers = []) {
    return servers.map((srv) => {
        ...
        return {
            ...
            name: srv.name,
            url: srv.url,
            headers: resolveNameValueEntries(srv.headers || [], {
                filterEmptyBearer: true,
            }),
        };
    });
}
...
// Default patterns that classify a provider/network error as transient and worth retrying.
// Each entry is a case-insensitive regex substring tested against the error message.
...
const AGENT_TIMEOUT_MS = pick(
    cfg.agentTimeoutMs,
    process.env.AGENT_TIMEOUT_MS,
    300_000,
);
const MAX_RETRIES = pick(cfg.maxRetries, process.env.MAX_RETRIES, 3);
const RETRY_DELAY_MS = pick(
    cfg.retryDelayMs,
    process.env.RETRY_DELAY_MS,
    5_000,
);
const HEARTBEAT_MS = pick(cfg.heartbeatMs, process.env.HEARTBEAT_MS, 30_000);
const MAX_RETRY_AFTER_MS = pick(
    cfg.maxRetryAfterMs,
    process.env.MAX_RETRY_AFTER_MS,
    5 * 60_000,
);
const MAX_LINE_BYTES = pick(
    cfg.maxLineBytes,
    process.env.MAX_LINE_BYTES,
    4 * 1024 * 1024,
);
const MAX_OUTPUT_BYTES = pick(
    cfg.maxOutputBytes,
    process.env.MAX_OUTPUT_BYTES,
    10 * 1024 * 1024,
);
const REVIEWER_AGENT_CHARS = pick(
    cfg.reviewerAgentChars,
    process.env.REVIEWER_AGENT_CHARS,
    40_000,
);
const CONCURRENCY = pick(cfg.concurrency, process.env.CONCURRENCY, 4);
const PROBE_TIMEOUT_MS = pick(
    cfg.probeTimeoutMs,
    process.env.PROBE_TIMEOUT_MS,
    20_000,
);
const RETRYABLE_PATTERNS =
    cfg.retryablePatterns != null
        ? cfg.retryablePatterns
        : DEFAULT_RETRYABLE_PATTERNS;

// Debug logging: when enabled, every JSON-RPC frame and every stderr line from each
// sub-agent is mirrored to the orchestrator's own stderr (which Zed surfaces under
// "agent server logs" / `dev: open acp logs` as stderr entries). Enable via
// cfg.debug=true or by setting ORCHESTRATOR_DEBUG to any non-empty value other
// than "0"/"false".
const DEBUG_LOG = (() => {
    if (typeof cfg.debug === "boolean") return cfg.debug;
    const env = process.env.ORCHESTRATOR_DEBUG;
...
function assertWorkspaceRelativePath(name, val) {
    ...
    if (path.isAbsolute(val) || path.win32.isAbsolute(val)) {
        fail(
            `${name} must be workspace-relative, got absolute path ${JSON.stringify(val)}`,
        );
    }
...
if (cfg.retryablePatterns !== undefined) {
    if (
        !Array.isArray(cfg.retryablePatterns) ||
        !cfg.retryablePatterns.length
    ) {
        fail("retryablePatterns must be a non-empty array of strings");
    }
    for (const [i, p] of cfg.retryablePatterns.entries()) {
        if (typeof p !== "string" || !p) {
            fail(`retryablePatterns[${i}] must be a non-empty string`);
        }
        try {
            new RegExp(p, "i");
        } catch {
            fail(
                `retryablePatterns[${i}] is not a valid regular expression: ${JSON.stringify(p)}`,
            );
        }
    }
}
...
if (cfg.defaultGroup !== undefined)
    assertString("defaultGroup", cfg.defaultGroup);
if (cfg.artifactDir !== undefined)
    assertWorkspaceRelativePath("artifactDir", cfg.artifactDir);
...
function validateAgentSpec(p, a) {
    ...
    if (a.rateLimitKey !== undefined)
        assertString(`${p}.rateLimitKey`, a.rateLimitKey);
    if (a.args !== undefined) assertStringArray(`${p}.args`, a.args);
    if (a.sandboxCommand !== undefined)
        assertString(`${p}.sandboxCommand`, a.sandboxCommand);
    if (a.sandboxArgs !== undefined)
        assertStringArray(`${p}.sandboxArgs`, a.sandboxArgs);
    ...
    if (
        a.credHome !== undefined &&
        (typeof a.credHome !== "string" || !a.credHome)
    ) {
...
function policyAllowsMutation(policy) {
    return Boolean(
        policy.writeFiles ||
        policy.terminal ||
        policy.allowUnknownClientRequests,
    );
}
...
function assertReadOnlyParallelReports(groupName, group) {
    ...
    const subPolicy = validatePermissions(
        `agentGroups.${groupName}.permissions`,
        group.permissions,
    );
    const reviewerPolicy = validatePermissions(
        `agentGroups.${groupName}.reviewerPermissions`,
        group.reviewerPermissions,
    );

    if (
        policyAllowsMutation(subPolicy) ||
        policyAllowsMutation(reviewerPolicy)
    ) {
        fail(
            `agentGroups.${groupName}: parallel_reports groups must be read-only; use single_writer for code-changing phases`,
        );
    }

    if (group.writer !== undefined || group.writerPermissions !== undefined) {
        fail(
            `agentGroups.${groupName}: writer and writerPermissions are only valid for single_writer groups`,
        );
    }
}
...
function assertReadOnlyReviewer(groupName, group) {
    ...
    if (policyAllowsMutation(reviewerPolicy)) {
        fail(
            `agentGroups.${groupName}.reviewerPermissions must be read-only; reviewers cannot receive write or terminal permissions`,
        );
    }
}
...
for (const [i, s] of (cfg.mcpServers || []).entries()) {
    ...
    if (!["http", "stdio"].includes(s.type)) {
        fail(
            `mcpServers[${i}].type must be "http" or "stdio", got ${JSON.stringify(s.type)}`,
        );
    }
    ...
    if (s.args !== undefined)
        assertStringArray(`mcpServers[${i}].args`, s.args);
    if (s.headers !== undefined)
        assertNameValueArray(`mcpServers[${i}].headers`, s.headers);
    if (s.env !== undefined)
        assertNameValueArray(`mcpServers[${i}].env`, s.env);
}

if (cfg.rateLimits !== undefined) {
    if (
        !cfg.rateLimits ||
        typeof cfg.rateLimits !== "object" ||
        Array.isArray(cfg.rateLimits)
    ) {
    ...
    for (const [command, limit] of Object.entries(cfg.rateLimits)) {
        ...
        if (
            !Number.isFinite(limit.requestsPerMinute) ||
            limit.requestsPerMinute <= 0
        ) {
            fail(
                `rateLimits.${command}.requestsPerMinute must be a number > 0`,
            );
        }
...
if (cfg.agentGroups !== undefined) {
    if (
        !cfg.agentGroups ||
        typeof cfg.agentGroups !== "object" ||
        Array.isArray(cfg.agentGroups)
    ) {
    ...
    for (const [name, group] of Object.entries(cfg.agentGroups)) {
        ...
        if (
            ...
            !["parallel_reports", "single_writer"].includes(group.strategy)
        ) {
            fail(
                `agentGroups.${name}.strategy must be "parallel_reports" or "single_writer"`,
            );
        }
        if (group.permissions !== undefined)
            validatePermissions(
                `agentGroups.${name}.permissions`,
                group.permissions,
            );
        if (group.reviewerPermissions !== undefined)
            validatePermissions(
                `agentGroups.${name}.reviewerPermissions`,
                group.reviewerPermissions,
            );
        if (group.writerPermissions !== undefined)
            validatePermissions(
                `agentGroups.${name}.writerPermissions`,
                group.writerPermissions,
            );
        if (group.artifactDir !== undefined)
            assertWorkspaceRelativePath(
                `agentGroups.${name}.artifactDir`,
                group.artifactDir,
            );
        if (group.attachApprovedPlanFrom !== undefined)
            assertString(
                `agentGroups.${name}.attachApprovedPlanFrom`,
                group.attachApprovedPlanFrom,
            );
        if (group.writer !== undefined)
            assertString(`agentGroups.${name}.writer`, group.writer);
        if (group.concurrency !== undefined)
            assertInt(`agentGroups.${name}.concurrency`, group.concurrency, 1);
        ...
        const duplicateName = agentNames.find(
            (agentName, idx) => agentNames.indexOf(agentName) !== idx,
        );
        if (duplicateName) {
            fail(
                `agentGroups.${name}.subAgents contains duplicate agent name ${JSON.stringify(duplicateName)}`,
            );
        }
        const strategy = group.strategy || "parallel_reports";
        ...
        if (strategy === "single_writer") {
            if (!group.writer) {
                fail(
                    `agentGroups.${name}.writer is required when strategy is "single_writer"`,
                );
            }
            const matches = agentNames.filter(
                (agentName) => agentName === group.writer,
            );
            if (matches.length !== 1) {
                fail(
                    `agentGroups.${name}.writer must match exactly one subAgents[].name, got ${matches.length} matches for ${JSON.stringify(group.writer)}`,
                );
            }
        }
        if (group.reviewer !== undefined)
            validateAgentSpec(`agentGroups.${name}.reviewer`, group.reviewer);
...
function buildAgentGroups() {
    ...
    for (const [name, group] of Object.entries(rawGroups)) {
        ...
        if (!reviewer) {
            fail(
                `agentGroups.${name}.reviewer is required when no top-level reviewer is configured`,
            );
        }
        groups[name] = {
            name,
            ...
            persist:
                group.persist ??
                (group.strategy || "parallel_reports") !== "single_writer",
            ...
            attachApprovedPlanFrom:
                group.attachApprovedPlanFrom ??
                (name === "code" && rawGroups.plan ? "plan" : undefined),
            artifactDir:
                group.artifactDir || cfg.artifactDir || ".plan/orchestrator",
...
const DEFAULT_GROUP =
    cfg.defaultGroup || (cfg.agentGroups ? GROUP_NAMES[0] : "default");
if (!AGENT_GROUPS[DEFAULT_GROUP]) {
    fail(
        `defaultGroup must reference an existing agentGroups entry, got ${JSON.stringify(DEFAULT_GROUP)}`,
    );
}
...
for (const [i, name] of WORKFLOW.entries()) {
    if (!AGENT_GROUPS[name]) {
        fail(
            `workflow[${i}] references unknown agent group ${JSON.stringify(name)}`,
        );
    }
}
for (const [name, group] of Object.entries(AGENT_GROUPS)) {
    if (
        group.attachApprovedPlanFrom &&
        !AGENT_GROUPS[group.attachApprovedPlanFrom]
    ) {
        fail(
            `agentGroups.${name}.attachApprovedPlanFrom references unknown agent group ${JSON.stringify(group.attachApprovedPlanFrom)}`,
        );
    }
}
