...
const {
  AGENT_GROUPS, DEFAULT_GROUP, MCP_SERVERS, AGENT_TIMEOUT_MS,
} = require('./config');
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = timeoutMs > 0
      ? setTimeout(() => {
          ...
          const err = new Error(`TIMEOUT:${timeoutMs}ms — ${method} did not respond`);
          err.code = -32000;
          reject(err);
        }, timeoutMs)
      : null;
...
rl.on('line', async line => {
  ...
  if (msg.method === 'initialize') {
    ...
    sessions.set(sessionId, {
      workDir:    msg.params?.cwd || msg.params?.workingDirectory || process.cwd(),
    ...
    if (!session) {
      send({ jsonrpc: '2.0', id: msg.id, error: { code: -32000, message: `Unknown session: ${sessionId}` } });
      return;
    }
    if (session.ctx) {
      send({ jsonrpc: '2.0', id: msg.id, error: { code: -32000, message: 'Session busy: a prompt is already in progress' } });
      return;
    }
    ...
    if (requestedGroup) {
      if (!AGENT_GROUPS[requestedGroup]) {
        send({ jsonrpc: '2.0', id: msg.id, error: { code: -32602, message: `Unknown orchestrator group: ${requestedGroup}` } });
        return;
      }
    ...
    log(`Task received (${task.length} block(s)), group=${group.name}, orchestrating…`);
    let finalResult;
    try {
      const outcome = await orchestrate(
        sessionId, runId, promptId, task, workDir, group, ctx,
        session.mcpServers, notifyText, zedClientCapabilities,
      );
      ...
      if (ctx.latestApprovedPlans) session.latestApprovedPlans = ctx.latestApprovedPlans;
      const activeGroup = AGENT_GROUPS[activeGroupName];
      if (activeGroup?.persist === false || activeGroup?.strategy === 'single_writer') {
  ...
  } else if (msg.method === 'session/release' || msg.method === 'session/close') {
    const { sessionId } = msg.params || {};
    const session = sessions.get(sessionId);
    if (session?.ctx) {
      session.ctx.cancelled = true;
...
async function shutdown(signal) {
  ...
  rejectPendingZedRequests(new Error(`orchestrator shutting down: ${signal}`));
